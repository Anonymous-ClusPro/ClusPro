...
class cluspro(nn.Module):
    ...
    def loss_calu(self, predict, target):
        ...
        if self.training:
            loss = loss_comp * self.config.pair_loss_weight +\
                ...
                loss_obj * self.config.obj_loss_weight + 0.1*loss_contras
        else:
            loss = loss_comp * self.config.pair_loss_weight +\
               loss_attr * self.config.attr_loss_weight +\
    ...
    def train_forward(self, batch, idx):
        ...
        batch_attr = self.attr_disentangler(batch_img)
        batch_obj = self.obj_disentangler(batch_img)
        results = []
        results1 = []
        ...
        attr_masks = torch.einsum('bd,kmd->bmk', l2_normalize(batch_attr), featattr_memory.detach().reshape(-1,self.cluter_num,768))
        obj_masks = torch.einsum('bd,kmd->bmk', l2_normalize(batch_obj), featobj_memory.detach().reshape(-1,self.cluter_num,768))
        ...
        attronehot.scatter_(1, attrindex, 1)
        ...
        objonehot.scatter_(1, objindex, 1)
        ...
        loss_hsic1 = hsic_normalized(batch_img_features[1], 0.5*batch_img_features[2]+0.5*select_objpro)
        loss_hsic2 =  0.5 *hsic_normalized(batch_img_features[2], 0.5*select_attrpro+0.5*batch_img_features[1])
